...
namespace NewAgeLauncher
{
    public partial class MainForm : Form
    {
        ...
        public MainForm()
        {
            ...
            if(Settings_ES.Default.TransparencyToggle == false)
            {
                Opacity = 1.00;
            }
            else
        ...
        private void MainForm_Load(object sender, EventArgs e)
        {
            ...
            if (Settings_ES.Default.LaunguageSet == true)
            {
            ...
            */
            ...
            if (Settings_ES.Default.TransparencyToggle == true)
            {
                Opacity = .90;
            }
            ...
            if (string.IsNullOrEmpty(Settings_ES.Default.WowLocation) || !Directory.Exists(Settings_ES.Default.WowLocation))
            {

                if (!string.IsNullOrEmpty(WoW.Directory))
                {
                    Settings_ES.Default.WowLocation = WoW.Directory;
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(path) && File.Exists(path))
                {
                    using (StreamWriter writer = new StreamWriter(path))
                    {
                        ...
                        writer.WriteLine(Settings_ES.Default.realmlist);
                        writer.Flush();
                ...
                if (Settings_ES.Default.patchDownloadURL != String.Empty)
        ...
        private void startDownloadBackgroundWorker_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            using (client = new WebClient())
            {
                ...
                try
                {
                    ...
                    client.DownloadFileAsync(new Uri(Settings_ES.Default.patchDownloadURL), tempPath);
                }
                catch (Exception ex)
                {
        ...
        private void checkServerStatusBackgroundWorker_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            using (TcpClient client = new TcpClient())
            {
                try
                {
                    client.Connect(Settings_ES.Default.server, Settings_ES.Default.port);
